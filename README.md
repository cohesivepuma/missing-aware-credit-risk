...
本仓库用于课程科研项目和论文实验，目标是建立结构清晰、可多人协作、可复现的个人信用风险研究框架。当前处于第一阶段：已实现离线 toy 数据的 Logistic Regression 最小流程；真实信用数据和高级研究模块将在后续 Issues 中逐步完成。
...
data = {
    ...
    "y": ...,     # int64, shape (n_samples,)，1 = 违约/坏信用，0 = 非违约/好信用
}
```
...
├── src/
│   ├── __init__.py
│   ├── data/                 # loader.py / preprocess.py / missing_generator.py
...
测试覆盖手算指标、概率边界、mask 约定、三个随机数生成器、固定种子划分与预测、划分互斥、缺失值填充和训练集预处理边界。高级缺失模块的测试只验证接口拒绝非法参数及明确告知尚未实现，并未验证 MCAR / MAR / MNAR 算法。
...
1. 接入公开信用数据，明确目标映射、特征类型、引用与下载校验，保存固定行索引划分；类别编码只在训练集拟合。
